 ...
 * butterflies, and angled light rays. Performance-tuned with capped particle
 * counts and devicePixelRatio handling. Not overdone — every effect is subtle.
 */
type Particle = {
  x: number;
  y: number;
...
export default function AmbientForest({
  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      particles.current = [];
      const flyCount = Math.floor(26 * density);
      const dustCount = Math.floor(45 * density);
      const leafCount = showLeaves ? Math.floor(6 * density) : 0;
      const butterflyCount = showButterflies ? Math.floor(4 * density) : 0;
    ...
    const draw = () => {
      ...
      // Fog bands (soft horizontal drifting mist)
      ...
      for (let i = 0; i < particles.current.length; i++) {
        ...
        if (p.type === 'firefly') {
          ...
          ctx.fill();
          ctx.strokeStyle = `rgba(82, 183, 136, ${alpha * 0.6})`;
          ctx.lineWidth = 0.6;
          ctx.beginPath();
          ctx.moveTo(-p.size, 0);
          ctx.lineTo(p.size, 0);
          ...
          // Upper wings
          ctx.beginPath();
          ctx.ellipse(-wingW * 0.5, -wingH * 0.3, wingW, wingH, -0.4, 0, Math.PI * 2);
          ctx.fill();
          ctx.beginPath();
          ctx.ellipse(-wingW * 0.5, wingH * 0.3, wingW * 0.8, wingH * 0.7, 0.4, 0, Math.PI * 2);
          ctx.fill();
          // Body
          ctx.fillStyle = 'rgba(120, 200, 160, 0.7)';
          ctx.beginPath();
          ctx.ellipse(0, 0, p.size * 0.15, p.size * 0.6, 0, 0, Math.PI * 2);
          ctx.fill();
          ctx.restore();
        }
      }
